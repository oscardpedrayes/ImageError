...
template <unsigned int PSIDE>
void Patch<PSIDE>::initGaussian(double sigma, bool normalize)
{
	const double sideMid = 0.5 * (PSIDE - 1);
	...
	for (unsigned int i = 0; i < PSIDE; ++i)
	{
		...
		{
			const double dist_j = j - sideMid;
			imagePatch_[i][j] = exp(K * (dist_i * dist_i + dist_j * dist_j));
			sum += imagePatch_[i][j];
		}
	}
	if (normalize)
...
template <unsigned int PSIDE>
const double *Patch<PSIDE>::getRow(unsigned int row) const
{
	return imagePatch_[row];
}

template <unsigned int PSIDE>
double *Patch<PSIDE>::getRow(unsigned int row)
{
	return imagePatch_[row];
}
...
void Patch<PSIDE>::readPatch(const Image &img,const unsigned int i, const unsigned int j)
{
	...
	avg_ = 0;
	dev_ = 0;
#if -1 // 0 => Media sin gauss - 1 => Media con Gauss
